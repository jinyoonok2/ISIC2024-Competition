...
class SCSAHead(nn.Module):
    def __init__(self, input_channels, backbone_output_size, device):
        ...
        self.output_layer = nn.Linear(512, 1).to(device)
        self.sigmoid = nn.Sigmoid()
...
def get_model_head(head_type, input_channels, backbone_output_size, device):
    ...
    elif head_type.lower() == 'lightgbm':
        # Return a placeholder, LightGBM will be handled separately in model_utils.py
        return None
    else:
        raise ValueError(f"Unknown head type: {head_type}. Please choose 'simplehead', 'simpleheadv2', 'scsa', or 'lightgbm'.")
