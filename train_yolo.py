...
class RegressionHead(nn.Module):
    def __init__(self, in_features):
        ...
        self.conv = nn.Conv2d(in_features, 1280, kernel_size=1, stride=1, bias=False)
        self.bn = nn.BatchNorm2d(1280)
        self.act = nn.SiLU(inplace=True)
        self.pool = nn.AdaptiveAvgPool2d(output_size=1)
        self.drop = nn.Dropout(p=0.0, inplace=True)
        self.linear = nn.Linear(1280, 1)  # Regression output a single value
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x, 1)
        x = self.drop(x)
        x = torch.sigmoid(self.linear(x))  # Sigmoid activation for probability output
        return x

# Define a wrapper around the YOLO model
class YOLORegression(nn.Module):
    def __init__(self, yolo_model):
        super(YOLORegression, self).__init__()
        self.features = nn.Sequential(*list(yolo_model.model.model.children())[:-1])
        in_features = yolo_model.model.model[-1].conv.conv.in_channels
...
def train_model(dataloader, config):
    ...
    # Load the YOLOv8 classification model and wrap it with YOLORegression
    yolo_model = YOLO('yolov8l-cls.yaml').load('yolov8l-cls.pt')
    model = YOLORegression(yolo_model)
