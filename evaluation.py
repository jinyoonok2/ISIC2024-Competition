...
config = {
    ...
    'lightgbm_model_path': './logs/lightgbm/best_model.txt',  # Updated to a single model path
    'catboost_model_path': './logs/catboost/best_model.cbm',  # Updated to a single model path
...
# Updated Function to Load a Single Model
...
# Updated Function to Make Predictions with Single LightGBM and CatBoost Models
def predict_test_data():
    ...
    # Load trained models
    lgb_model = load_model(config['lightgbm_model_path'], 'lightgbm')
    cb_model = load_model(config['catboost_model_path'], 'catboost')

    # Make predictions with the single LightGBM model
    lgb_preds = lgb_model.predict(df_test)

    # Make predictions with the single CatBoost model
    cb_preds = cb_model.predict_proba(df_test)[:, 1]
